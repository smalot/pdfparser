...
foreach ($tests as $test) { /** @var $test \Tests\Smalot\PdfParser\Performance\AbstractPerformanceTest */
    ...
    if ($test->getMaxEstimatedTime() < $time) {
        throw new \Tests\Smalot\PdfParser\Performance\PerformanceFailException(
            sprintf('Performance failed on test "%s". Time taken was %.2f seconds, expected less than %d seconds.',
                get_class($test),
                $time,
